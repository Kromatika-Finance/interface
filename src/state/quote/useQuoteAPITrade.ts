...
import { CHAIN_0x_URL, useGetSwap0xQuery, useGetSwapServerQuery } from 'state/quote/slice'
...
function useRoutingAPIArguments({
  ...
  return {
    ...
    queryArg: {
      ...
      fromAddress: account,
      slippage: allowedSlippage.divide(1_000).toSignificant(6),
...
export function use0xQuoteAPITrade(
  ...
  console.log('here 1')
  const { isLoading, isError, data } = useGetSwapServerQuery(queryArgs ?? skipToken, {
 ...
 * @param otherCurrency the desired output/payment currency
 */
export function useBetterTradeAPITrade(
  tradeType: TradeType,
  recipient: string | null | undefined,
  skipValidation: boolean,
  showConfirm: boolean,
